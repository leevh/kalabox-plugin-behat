...
module.exports = function(kbox, app) {
  ...
  app.events.on('post-app-load', function(app) {
    ...
    if (!fs.existsSync(path.resolve(__dirname, '../../..', './config/behat'))) {
      ...
      var settingsPath = path.resolve(__dirname, '../../..',
          'config/behat/settings.php');
      fs.copySync(path.resolve(__dirname, '..', 'settings.php'),
          settingsPath, {});
      ...
      fs.readFile(settingsPath, 'utf8', function(err, data) {
        ...
        var result = data.replace(/pantheon/g, app.env.state.KALABOX_APP_PANTHEON_FRAMEWORK);
